...
const OUTPUT_FILE = `${OUTPUT_DIR}/serp_app_embeddings.jsonl`;
const PROGRESS_FILE = `${OUTPUT_DIR}/serp_progress.json`;
const BATCH_SIZE = 20;
...
async function generateSerpEmbeddings() {
  console.log('Starting SERP local embedding generation...');
  ...
  while (true) {
    console.log(`\nFetching batch of apps from serp_unique_apps after ID ${lastProcessedId}...`);
  ...
  console.log(`\n✅ Full SERP embedding generation complete. Total apps processed: ${appsProcessed}`);
}

generateSerpEmbeddings();
