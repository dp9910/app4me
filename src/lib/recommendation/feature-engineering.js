 ...
 * Extracts 5 types of features from app data for advanced recommendations
 */
...
// Category keyword mappings (enhanced from your original plan)
 ...
 */
export async function extractTFIDFKeywords(text, maxKeywords = 20) {
  ...
  // Calculate TF-IDF scores (simplified - using just TF for now)
...
/**
 * Classify app into categories based on keywords and metadata
 */
...
/**
 * Helper functions for metadata features
 */
 ...
 */
export async function batchEnrichApps(apps, batchSize = 10) {
  const results = [];
